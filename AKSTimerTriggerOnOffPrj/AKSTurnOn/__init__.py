...
def main(mytimer: func.TimerRequest) -> None:
    ...
    logging.info("Starting AKS...")
    try:
        req = requests.post(
            "https://management.azure.com/subscriptions/"+subscription_id+
            "/resourceGroups/"+resourceGroupName+
            "/providers/Microsoft.ContainerService/managedClusters/"+resourceName+
            "/start?api-version=2022-04-01",headers=headers
        )

        logging.info("Response start AKS http status: "+str(req.status_code))
    except Exception as e:
        logging.info("Error al llamar API de Start AKS")
        logging.info(str(e))
